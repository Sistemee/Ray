...
enum TOKENS {
	...
	DECS_T      /* 12: Decimal separator token*/
};
...
typedef union TokenAttribute {
	ray_intg codeType;      /* integer attributes accessor */
	AriOperator arithmeticOperator;		/* arithmetic operator attribute code */
	RelOperator relationalOperator;		/* relational operator attribute code */
	LogOperator logicalOperator;		/* logical operator attribute code */
	EofOperator seofType;				/* source-end-of-file attribute code */
	ray_intg intValue;						/* integer literal attribute (value) */
	ray_intg keywordIndex;					/* keyword index in the keyword table */
	ray_intg contentString;				/* string literal offset from the beginning of the string literal buffer (stringLiteralTable->content) */
	ray_real floatValue;					/* floating-point literal attribute (value) */
	ray_char idLexeme[VID_LEN + 1];		/* variable identifier token attribute */
	ray_char errLexeme[ERR_LEN + 1];		/* error token attribite */
} TokenAttribute;

/* TO_DO: Should be used if no symbol table is implemented */
typedef struct idAttibutes {
	ray_byte flags;			/* Flags information */
	union {
		ray_intg intValue;				/* Integer value */
		ray_real floatValue;			/* Float value */
		ray_char* stringContent;		/* String value */
	} values;
} IdAttibutes;

/* Token declaration */
typedef struct Token {
	ray_intg code;				/* token code */
...
ray_intg startScanner(ReaderPointer psc_buf);
Token tokenizer(ray_void);
static ray_intg nextClass(ray_char c);				/* character class function */
static ray_intg nextState(ray_intg, ray_char);		/* state machine function */
ray_void printToken(Token t);
...
typedef Token(*PTR_ACCFUN)(ray_char* lexeme);

/* Declare accepting states functions */
Token funcSL	(ray_char lexeme[]);
...
typedef struct languageAttributes {
	ray_char indentationCharType;
	ray_intg indentationCurrentPos;
	/* TO_DO: Include any extra attribute to be used in your scanner (OPTIONAL and FREE) */
} LanguageAttributes;

/* Number of errors */
ray_intg numScannerErrors;
