...
ray_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern ray_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern ray_intg line; /* source code line number - defined in scanner.c */
...
extern ray_void startParser(ray_void);
extern ray_intg startScanner(ReaderPointer sc_buf);

static ray_void printParserError(ray_char* fmt, ...);
static ray_void displayParser(ReaderPointer ptrBuffer);
static ray_long getParserFileSize(ray_char* fname);
static ray_void callGarbageCollector(ray_void);
...
ray_void printParserError(ray_char* fmt, ...) {
...
ray_long getParserFileSize(ray_char* fname) {
	FILE* input;
	ray_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
ray_void displayParser(ReaderPointer ptrBuffer) {
...
ray_void callGarbageCollector(julius_void) {
